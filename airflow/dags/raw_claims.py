...
FILE_PATH = "/opt/airflow/dags/customer_1_inpatient_20200101.csv"
#FILE_PATH = r"D:\python_Scripts\customer_1_inpatient.csv"
...
def check_date_quality_and_validate(**kwargs):
    ...
    print(f"Expected date extracted from file name: {expected_date}")
    ...
    print(f"Last date of the month: {last_date_of_month}")
    # Query Snowflake
    snowflake_hook = SnowflakeHook(snowflake_conn_id="snowflake_default")
    clean_name = file_name.replace(".gz", "")
    sql = f"""
    SELECT
    ...
    FROM
        raw_claims
    WHERE
        source_file_name LIKE '%{clean_name}%';
    """
    ...
    if not (min_date and max_date and min_date <= expected_date <= max_date):
        raise AirflowException(
